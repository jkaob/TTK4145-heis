...
class OrderNode(Node):
    ...
    def init_callback(self,msg):
        ...
        node_msg = messages.messages_createNodeMsg(elev)
...
#################### ORDER DISTRIBUTER ASSIGNS NEW ORDER #########################
    ...
    ###################################################################################
            ...
            if (elev.id == min_id):
                ...
                order_confirmed_msg = messages.messages_createOrderConfirmedMsg(elev, msg.floor, msg.button)
                ...
                if (elev.behaviour[elev.id] == constants.DOOR_OPEN and elev.floor[elev.id] == msg.floor):
                    ...
                    order_OrderExecutedMsg = messages.messages_createOrderExecutedMsg(elev)
                    self.order_executed_publisher.publish(order_OrderExecutedMsg)

                status_msg = messages.messages_createStatusMsg(elev)
        ...
        for f in range(constants.N_FLOORS):
            for b in range(constants.N_BUTTONS):
                ...
                if(v and (v != elev.queue[elev.id][f][b])):
                    ...
                    order_newOrderMsg = messages.messages_createOrderConfirmedMsg(elev,f,b)
