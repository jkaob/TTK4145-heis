...
def util_orderAbove(e): #Ta inn status klassen til seg selv
    for f in range(e.floor[e.id] + 1, constants.N_FLOORS):
...
def util_shouldStop(e):
    ...
    if (dir == constants.DIRN_DOWN):
        return ((e.queue[e.id][e.floor[e.id]][constants.BTN_HALL_DOWN]) or
                (e.queue[e.id][e.floor[e.id]][constants.BTN_CAB]) or
                not(util_orderBelow(e)))
    elif (dir == constants.DIRN_UP):
        return ((e.queue[e.id][e.floor[e.id]][constants.BTN_HALL_UP]) or
                (e.queue[e.id][e.floor[e.id]][constants.BTN_CAB]) or
