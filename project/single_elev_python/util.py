...
def util_shouldStop(e):
    dir = e.direction
    if (dir == DIRN_DOWN):
        return ((e.queue[ip_self][e.floor][BTN_HALL_DOWN]) or
                (e.queue[ip_self][e.floor][BTN_CAB]) or
                not(util_orderBelow(e)))
    else if (dir == DIRN_UP):
        return ((e.queue[ip_self][e.floor][BTN_HALL_UP]) or
                (e.queue[ip_self][e.floor][BTN_CAB]) or
